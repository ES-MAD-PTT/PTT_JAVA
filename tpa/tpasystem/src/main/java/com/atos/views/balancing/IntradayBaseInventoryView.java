...
import com.atos.services.balancing.IntradayBaseInventoryService;
import com.atos.utils.Constants;
import com.atos.utils.DateUtil;
import com.atos.views.CommonView;

@ManagedBean(name = "intradayBaseInvView")
@ViewScoped
public class IntradayBaseInventoryView extends CommonView implements Serializable {
	...
	private static final Logger log = LogManager.getLogger("com.atos.views.balancing.IntradayBaseInventory");
	private IntradayBaseInventoryFilter filters;
	private List<IntradayBaseInventoryBean> resultList;

	@ManagedProperty("#{intradayBaseInventoryService}")
	transient private IntradayBaseInventoryService service;

	public List<IntradayBaseInventoryBean> getResultList() {
		return resultList;
	}

	public void setResultList(List<IntradayBaseInventoryBean> resultList) {
	...
	public void init() {
		//super.setHead(8);
		filters = new IntradayBaseInventoryFilter();
	...
	private boolean validate() {
		...
		if(filters.getStrLastVersion().equals("N")) {
			if(filters.getTimestampVar().equals("")) {
				getMessages().addMessage(Constants.head_menu[8],
						new MessageBean(Constants.ERROR, msgs.getString("filter_error"),
								"Last Version or a timestamp must be selected", Calendar.getInstance().getTime()));
				return false;
			}
		}
		return true;
	...
	public void postProcessXLS(Object document) {
		...
		XSSFCellStyle cellStyleFourDecRed = createStyle(wb, contentFont, XSSFCellStyle.ALIGN_LEFT, Color.RED, true,
				Color.RED, XSSFCellStyle.BORDER_THIN, false, format.getFormat("#,##0.0000"));
		XSSFCellStyle cellStyleFourDecOrange = createStyle(wb, contentFont, XSSFCellStyle.ALIGN_LEFT, Color.ORANGE, true,
				Color.ORANGE, XSSFCellStyle.BORDER_THIN, false, format.getFormat("#,##0.0000"));
		XSSFCellStyle cellStyleFourDecYellow = createStyle(wb, contentFont, XSSFCellStyle.ALIGN_LEFT, Color.YELLOW, true,
				Color.YELLOW, XSSFCellStyle.BORDER_THIN, false, format.getFormat("#,##0.0000"));
				...
				for (j = 0; j < row.getPhysicalNumberOfCells(); j++) {
					...
					if (j == 0 || j == 1 || j == 2) { // Estas
						...
						if(allTotalRows.contains(i))
							...
							cell.setCellStyle(cellStyleFourDecRed);
						else if (j == 6 || j == 9)
							cell.setCellStyle(cellStyleFourDecOrange);
						else if (j == 7 || j == 8)
							cell.setCellStyle(cellStyleFourDecYellow);
						else
	...
	public Map<BigDecimal, Object> getZoneIds() {
		return service.selectZoneIds(filters.getSystemId());
	}
	
	public Map<BigDecimal, Object> getModeIds() {
		if(filters.getZoneId()!=null)
			return service.selectModeIds(filters.getZoneId());
		return null;		
	}
	...
	public IntradayBaseInventoryFilter getFilters() {
		return filters;
	}

	public void setFilters(IntradayBaseInventoryFilter filters) {
		this.filters = filters;
	}

	public IntradayBaseInventoryService getService() {
		return service;
	}

	public void setService(IntradayBaseInventoryService service) {
