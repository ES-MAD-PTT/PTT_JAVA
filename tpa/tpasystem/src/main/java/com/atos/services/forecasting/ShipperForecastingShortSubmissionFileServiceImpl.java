...
@Service("shipperForecastingShortSubmissionFileService")
public class ShipperForecastingShortSubmissionFileServiceImpl implements ShipperForecastingShortSubmissionFileService{	
	/**
	 * 
	 */
	private static final long serialVersionUID = -6627841585422654814L;
	...
	public BigDecimal selectOperationTerm() {
		List<BigDecimal> bd = dailyForMapper.selectOperationTerm(Constants.SHORT);
		if(bd.size()==0){
			return null;
		} else {
			return bd.get(0);
	...
	public String saveFile(ShipperForecastingSubmissionFileFilter filters, FileBean file) throws Exception{
		...
		ValidateIntervalBean valForecastingInterval = getValidateForecastingShortInterval(interval_start);
		...
		vib.setTerm_code(Constants.SHORT);
		...
		vdb.setTerm_code(Constants.SHORT);
		...
		// the operation "FORECASTING" + "SHORT"
	...
	public ValidateIntervalBean getValidateForecastingShortInterval(Date interval_start) {
		
		ValidateIntervalBean bean = new ValidateIntervalBean();
		bean.setCategory_code(Constants.FORECASTING);
		bean.setTerm_code(Constants.SHORT);
