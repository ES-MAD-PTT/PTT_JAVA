...
import com.atos.client.pmisdwh.acuminventory.AcumInventoryWsClient;
...
import com.atos.wsdl.pmisdwh.acuminventory.CdmError;
import com.atos.wsdl.pmisdwh.acuminventory.CdmRequestHeader;
import com.atos.wsdl.pmisdwh.acuminventory.CdmResponseHeader;
import com.atos.wsdl.pmisdwh.acuminventory.DateRange;
import com.atos.wsdl.pmisdwh.acuminventory.MeteringQueryRequest;
import com.atos.wsdl.pmisdwh.acuminventory.MeteringQueryResponse;
import com.atos.wsdl.pmisdwh.acuminventory.ObjectFactory;
import com.atos.wsdl.pmisdwh.acuminventory.ProcessMessage;
import com.atos.wsdl.pmisdwh.acuminventory.ProcessMessageCls;
import com.atos.wsdl.pmisdwh.acuminventory.ProcessMessageResponse;


@Component
public class AcumInventoryAutorunClient  implements Serializable {
	
	
	private static final long serialVersionUID = -9067325632463736509L;
	private static final Logger log = LogManager.getLogger(AcumInventoryAutorunClient.class);
	private static final String strNotifSeparator = "~";

	private static final String strAcumInventoryAutorun = "AUTORUN.ACUM.INVENTORY";
	...
	@Autowired
	private AcumInventoryWsClient wsClient;
	...
	public void callAcumInventoryClient() throws Exception{
		System.out.println("Job callAcumInventoryClient is runing");
		log.info("Job callAcumInventoryClient is runing", Calendar.getInstance().getTime());
...
//		ResourceBundle msgs = FacesContext.getCurrentInstance().getApplication().getResourceBundle(FacesContext.getCurrentInstance(),"msg");
    	...
    	bTakeLock = takeLockAcumInventory(strUser);
    	...
    	if(!bTakeLock)
    		...
    		callAcumInventoryFromWebserviceTask();
        }   
			...
			if(bTakeLock) {
				...
				if(!releaseLockAcumInventory(strUser))
	...
	public void  callAcumInventoryFromWebserviceTask() {
		
		
		//buscamos el flag que nos indique si enviamos notificaciones o no...
		String execution_enable="N";
		try {
			execution_enable = getSystemParameterString(strAcumInventoryAutorun, strUser, strLanguage);
		} catch (Exception e) {
	    	log.error(e.getMessage(), e);
		}
		if(!execution_enable.equals("Y")) {
			System.out.println("Job callAcumInventoryFromWebserviceTask is disable");
			log.info("Job callAcumInventoryFromWebserviceTask is disable", Calendar.getInstance().getTime());
			return;
		}
...
/*    	BigDecimal count = wsMapper.checkIntradayExecution(this.idnOnshoreSystem);
        ...
        try {
	        ...
	        if(isOKResult(response)) {
		    	...
		    	notifInfo += strNotifSeparator + "Webservice Input " + Constants.ACUM_WEBSERVICE;
        	...
        	// Se libera el bloqueo para que se pueda volver a ejecutar el proceso mas adelante.
            if(!releaseLockAcumInventory(strUser))
    	    	log.error("There has not been able to release the dababase lock on " + Constants.ACUM_WEBSERVICE +" process.");
	...
	private WebserviceLogBean wsLogInBDRequest(Date startDate, ProcessMessage msg) {
		...
		try {
			tmpWsLogBean.setWebserviceName(Constants.ACUM_INVENTORY);
	...
	private boolean takeLockAcumInventory(String userId) {
	...
	private boolean releaseLockAcumInventory(String userId) {
    	LockBean tmpLock = new LockBean();
    	tmpLock.setProcessCode(Constants.ACUM_INVENTORY);
    	tmpLock.setUserId(userId);
