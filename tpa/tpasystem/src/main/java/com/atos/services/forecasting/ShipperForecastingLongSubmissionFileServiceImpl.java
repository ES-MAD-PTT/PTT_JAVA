...
@Service("shipperForecastingLongSubmissionFileService")
public class ShipperForecastingLongSubmissionFileServiceImpl implements ShipperForecastingLongSubmissionFileService{
	/**
	 * 
	 */
	private static final long serialVersionUID = -7369269127056764724L;
	...
	public BigDecimal selectOperationTerm() {
		List<BigDecimal> bd = dailyForMapper.selectOperationTerm(Constants.LONG);
		if(bd.size()==0){
			return null;
		} else {
			return bd.get(0);
	...
	public String saveFile(ShipperForecastingSubmissionFileFilter filters, FileBean file) throws Exception{
    	...
    	ValidateIntervalBean valForecastingInterval = getValidateForecastingLongInterval(interval_start);
		...
		vib.setTerm_code(Constants.LONG);
		...
		vdb.setTerm_code(Constants.LONG);
		...
		// the operation "FORECASTING" + "LONG"
	...
	public ValidateIntervalBean getValidateForecastingLongInterval(Date interval_start) {
		...
		bean.setTerm_code(Constants.LONG);
