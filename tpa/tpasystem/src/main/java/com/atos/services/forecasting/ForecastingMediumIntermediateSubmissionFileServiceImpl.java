...
@Service("forecastingMediumIntermediateSubmissionFileService")
public class ForecastingMediumIntermediateSubmissionFileServiceImpl implements ForecastingMediumIntermediateSubmissionFileService{
	...
	private static final Logger log = LogManager.getLogger(ForecastingMediumIntermediateSubmissionFileServiceImpl.class);
	...
	public BigDecimal selectOperationTerm() {
		List<BigDecimal> bd = dailyNomMapper.selectOperationTerm(Constants.MEDIUM);
		if(bd.size()==0){
			return null;
		} else {
			return bd.get(0);
	...
	public HashMap<String,Object> saveFile(FileBean file,String user, BigDecimal systemId) throws Exception{
		...
		if(xmlConverter==null){
			...
			xmlConverter.init("FORECASTING", "MEDIUM", systemId);
    	}       	
		...
		// the operation "FORECASTING" + "MEDIUM" does not exists
		...
		vfx.setParType(Constants.MEDIUM);
		vfx.setUser(user);
