...
import com.atos.services.forecasting.ForecastingShortIntermediateSubmissionFileService;
import com.atos.utils.Constants;
import com.atos.views.CommonView;

@ManagedBean(name="forecastingShortIntermediateSubmissionFileView")
@ViewScoped
public class ForecastingShortIntermediateSubmissionFileView extends CommonView {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7323468912521183847L;

	private static final Logger log = LogManager.getLogger(ForecastingShortIntermediateSubmissionFileView.class);

	private UploadedFile file;
	private FileBean uploadFile;
	
	@ManagedProperty("#{forecastingShortIntermediateSubmissionFileService}")
    transient private ForecastingShortIntermediateSubmissionFileService service;
    
    public void setService(ForecastingShortIntermediateSubmissionFileService service) {
	...
	public void handleFileUpload(FileUploadEvent event) {
		...
		if(file==null || uploadFile==null){
			//messages.addMessage(Constants.head_menu[4],new MessageBean(Constants.ERROR,"Error saving file","The forecasting file should be selected", Calendar.getInstance().getTime()));
