...
@Component
public class AllocationAutorunClient  implements Serializable {
	
	
	private static final long serialVersionUID = -9067325632463736509L;
	private static final Logger log = LogManager.getLogger(AllocationAutorunClient.class);
	...
	private static final String strNotifTypeCodeWsMeteringManagementAutorun ="AUTORUN.METERING.MANAGEMENT";
	...
	private static final String strAllocationCommercialAllocation = "AUTORUN.COMMERCIAL.ALLOCATION";
	...
	public void callMeteringManagementRequestClient() throws Exception{
		System.out.println("Job callMeteringManagementRequestClient is runing");
		log.info("Job callMeteringManagementRequestClient is runing", Calendar.getInstance().getTime());
...
//		ResourceBundle msgs = FacesContext.getCurrentInstance().getApplication().getResourceBundle(FacesContext.getCurrentInstance(),"msg");
		...
		filter.setGasDayFrom(ayer.getTime());
    	...
    	if(!bTakeLock) {
    		...
    		String notifInfo = sdf.format(ayer.getTime()) + strNotifSeparator + "Error: There is another metering process in progress. Please, try it later.";	  
    		...
    		updateMeasurementsFromWebserviceTask();
        }   
	...
	public void  updateMeasurementsFromWebserviceTask() {
...
/*		String execution_enable="N";
	...
	*/	
	...
	//	ResourceBundle msgs = FacesContext.getCurrentInstance().getApplication().getResourceBundle(FacesContext.getCurrentInstance(),"msg");
    	...
    	cal_gasDayFrom.add(Calendar.DAY_OF_YEAR, -3);
    	
    	Calendar cal_gasDayTo = Calendar.getInstance();
    	cal_gasDayTo.add(Calendar.DAY_OF_YEAR, -1);
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	...
	public void  callAlocationCommercialRequestClient()throws JobExecutionException {
		System.out.println("Job callAlocationCommercialRequestClient is runing");
		log.info("Job callAlocationCommercialRequestClient is runing", Calendar.getInstance().getTime());

		//buscamos el flag que nos indique si enviamos notificaciones o no...
		String execution_enable="N";
		try {
			execution_enable = getSystemParameterString(strAllocationCommercialAllocation, strUser, strLanguage);
		} catch (Exception e) {
	    	log.error(e.getMessage(), e);
		}
		if(!execution_enable.equals("Y")) {
			System.out.println("Job callAlocationCommercialRequestClient is disable");
			log.info("Job callAlocationCommercialRequestClient is disable", Calendar.getInstance().getTime());
			return;
		}
		
		try {
			// llamamos al metodo para obtener las medidas
			callMeteringManagementRequestClient();
		} catch(Exception e) {
			log.error(e.getMessage(),e);
    	...
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
    	SimpleDateFormat sdf2 = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
    	String notifInfo = sdf.format(responsePeriodStartDate) + strNotifSeparator +
				sdf.format(responsePeriodEndDate) + strNotifSeparator +
				sdf2.format(startUpdatingDate);	    	
