...
import com.atos.services.nominations.ShipperDailySubmissionFileService;
import com.atos.utils.Constants;
import com.atos.views.MessagesView;

@ManagedBean(name="shipperDailySubmissionFileView")
@ViewScoped
public class ShipperDailySubmissionFileView implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1804211253281868923L;
	private static final Logger log = LogManager.getLogger(ShipperDailySubmissionFileView.class);
	...
	@ManagedProperty("#{shipperDailySubmissionFileService}")
    transient private ShipperDailySubmissionFileService service;
    
    public void setService(ShipperDailySubmissionFileService service) {
	...
	public void handleFileUpload(FileUploadEvent event) {
		if(filters.getStart_date()==null){
			messages.addMessage(Constants.head_menu[4],new MessageBean(Constants.ERROR,"Error saving file","Gas Day and Contract should be selected", Calendar.getInstance().getTime()));
			return;
		}
		Calendar cal = Calendar.getInstance();
		...
		if(error!=null && error.equals("-1")){
			messages.addMessage(Constants.head_menu[4],new MessageBean(Constants.ERROR,"Error saving file","The operation NOMINATION + DAILY does not exist", Calendar.getInstance().getTime()));			
			...
			String[] tmpaString = error.split(ShipperDailySubmissionFileService.errorSeparator, 2);
